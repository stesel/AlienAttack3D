	...
	public class Main extends Sprite 
	{
		...
		private function initShaders():void 
		{
			...
			//(
				//Context3DProgramType.FRAGMENT,
				//"tex ft0 v1, fs <2d,repeat,miplinear>\n" +	//grab the texture color from texture 0 and uv coordinates from v1
				//"mul ft1, fc0, ft0\n" +						//multiply by the value stored in fc1
				//"mov oc, ft1\n"								//move ft1 to oc
			//);
			...
			for (looptemp = 0; looptemp < 4; looptemp++)
			{
				...
				switch(looptemp)
				{
					...
					case 3:
						...
						context3D.setProgramConstantsFromVector(Context3DProgramType.FRAGMENT, 0, Vector.<Number>([1, Math.abs(Math.cos(t / 50)) * 0.4, 0, 1]));
						context3D.setTextureAt(0, myTexture);
